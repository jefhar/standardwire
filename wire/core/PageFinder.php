...
use Override;
use PDO;
use ProcessWire\Interfaces\FieldtypeLanguageInterface;
...
class PageFinder extends Wire {
	...
	protected $defaultOptions = [
      ...
      * 
      */
     'returnAllColsOptions' => [
         'joinFields' => [],
         // names of additional fields to join
         'joinSortfield' => false,
         // include 'sortfield' in returned columns? (joined from pages_sortfields table)
         'joinPath' => false,
         // include the 'path' in returned columns (joined from pages_paths table, requires PagePaths module)
         'getNumChildren' => false,
         // include 'numChildren' in returned columns? (sub-select from pages table)
	 ...
	 * 
	 */
	protected $extraOrSelectors = []; // one from each field must match
	 ...
	 * 
	 */
	protected $singlesFields = ['has_parent', 'hasParent', 'num_children', 'numChildren', 'children.count', 'limit', 'start'];
 ...
 public function __get($name) {
	...
	protected function initSelectors(Selectors $selectors, array $options) {
		...
		$startSelector = null;
		$addSelectors = [];
		$hasParents = []; // requests for parent(s) in the selector
		$hasSort = false; // whether or not a sort is requested
		
		// field names that do not accept array values
		$noArrayFields = [
      'status' => 1,
      // 1: array not allowed for field only
      'include' => 2,
      // 2: array not allowed for field or value
      'check_access' => 2,
      'checkAccess' => 2,
      'limit' => 1,
      'start' => 2,
      ...
      'get_total' => 2,
		...
		$includeOptions = ['hidden' => 'findHidden', 'unpublished' => 'findUnpublished', 'trash' => 'findTrash', 'all' => 'findAll'];
	...
	protected function initStatus(Selectors $selectors, array $options) {
		...
		$maxStatus = null;
		$lessStatus = 0;
		$statuses = []; // i.e. [ 'hidden' => 1024, 'unpublished' => 2048, ], etc
			...
			if($options['loadPages']) {
				...
				while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
					...
					if($stopBeforeID && $row['id'] == $stopBeforeID) {
						if($options['findOne']) {
							$matches = count($matches) ? [end($matches)] : [];
	 ...
	 *
	 */
	public function findIDs($selectors, $options = []) {
	 ...
	 * 
	 */
	public function findVerboseIDs($selectors, $options = []) {
	 ...
	 *
	 */
	public function findParentIDs($selectors, $options = []) {
		$options['returnVerbose'] = false;
	 ...
	 * 
	 */
	public function findTemplateIDs($selectors, $options = []) {
		$options['returnVerbose'] = false;
	 ...
	 * 
	 */
	public function count($selectors, $options = []) {
		
		$defaults = ['getTotal' => true, 'getTotalType' => 'count', 'loadPages' => false, 'returnVerbose' => false];
	 ...
	 * 
	 */
	protected function preProcessSelectors(Selectors $selectors, $options = []) {
	...
	protected function preProcessSelector(Selector $selector, Selectors $selectors, array $options, $level = 0) {
		...
		if($quote == '[') {
			...
			if(!isset($this->extraOrSelectors[$groupName])) $this->extraOrSelectors[$groupName] = [];
	...
	*/
	...
	protected function preProcessSubSelector(Selector $selector, Selectors $parentSelectors) {
		...
		if(!$hasTemplate || !$hasParent) {
			...
			$fields = is_array($selector->field) ? $selector->field : [$selector->field];
			$templates = [];
			$parents = [];
			$findSelector = '';
		...
		$natives = ['parent', 'parent.id', 'parent_id', 'children', 'children.id', 'child', 'child.id'];
	...
	protected function ___getQuery($selectors, array $options) {
		...
		$where = '';
		$fieldCnt = []; // counts number of instances for each field to ensure unique table aliases for ANDs on the same field
		$lastSelector = null; 
		$sortSelectors = []; // selector containing 'sort=', which gets added last
		$subqueries = [];
		$joins = [];
		$database = $this->database;
		$autojoinTables = [];
		...
		if($options['returnAllCols']) {
			...
			if(!empty($opts['joinFields'])) {
				$this->pageArrayData['joinFields'] = []; // identify whether each field supported autojoin
			...
			$columns = ['pages.id', 'pages.parent_id', 'pages.templates_id'];
		} else if($options['returnParentIDs']) {
			$columns = ['pages.parent_id AS id'];
		} else if($options['returnTemplateIDs']) {
			$columns = ['pages.id', 'pages.templates_id'];
		} else {
			$columns = ['pages.id'];
		}
		...
		foreach($selectors as $selector) {
			...
			foreach($fields as $fieldName) {
				...
				if(!$field) {
					...
					$field = $this->getQueryUnknownField($fieldName, ['subfield' => $subfield, 'subfields' => $subfields, 'fields' => $fields, 'query' => $query, 'selector' => $selector, 'selectors' => $selectors]);
					if($field === true) {
				...
				foreach($valueArray as $value) {
					...
					$q->copyTo($query, ['select', 'join', 'leftjoin', 'orderby', 'groupby']); 
				...
				if($join) {
					...
					if(isset($joins[$tableAlias])) {
						...
						$joins[$tableAlias] = ['joinType' => $joinType, 'table' => $field->table, 'tableAlias' => $tableAlias, 'join' => "($join)"];
					}
		...
		if(count($this->extraOrSelectors)) {
			...
			foreach($this->extraOrSelectors as /* $groupName => */ $selectorGroup) {
				...
				foreach($selectorGroup as $selectors) {
					$pageFinder = $this->wire(new PageFinder());	
					/** @var DatabaseQuerySelect $query */
					$query = $pageFinder->find($selectors, ['returnQuery' => true, 'returnVerbose' => false, 'findAll' => true, 'bindOptions' => ['prefix' => 'pfor', 'global' => true]]);
					if($n > 0) $sql .= " \n\tOR pages.id IN (\n";
					$query->set('groupby', []);
					$query->set('select', ['pages.id']); 
					$query->set('orderby', []); 
					$sql .= tabIndent("\t\t" . $query->getQuery() . "\n)", 2);
					$query->copyBindValuesTo($parentQuery, ['inSQL' => $sql]);
					$n++;
				}
				$sqls[] = $sql;
			}
	...
	protected function getQuerySortSelector(DatabaseQuerySelect $query, Selector $selector) {
		...
		$values = is_array($selector->value) ? $selector->value : [$selector->value]; 	
		$fields = $this->fields;
		$pages = $this->pages;
		$database = $this->database;
		$user = $this->wire()->user; 
	...
	protected function getQueryNativeField(DatabaseQuerySelect $query, $selector, $fields, array $options, $selectors) {
		...
		foreach($fields as $field) { 
			...
			if($isParent || $isChildren || $isPages) {
				...
				if(($isPages || $isParent) && !$isPartialOperator && (!$subfield || in_array($subfield, ['id', 'path', 'url']))) {
			...
			if(count($IDs)) {
				...
				$in = $not ? "NOT IN" : "IN";
				$sql .= in_array($field, ['parent', 'parent_id']) ? "$table.parent_id " : "$table.id ";
				$IDs = $sanitizer->intArray($IDs, ['strict' => true]);
				...
				if(in_array($field, ['templates_id', 'template'])) {
				...
				} else if(in_array($field, ['created', 'modified', 'published'])) {
				...
				} else if(in_array($field, ['id', 'parent_id', 'templates_id', 'sort'])) {
					$value = (int) $value; 
				}
	...
	protected function getQueryHasParent(DatabaseQuerySelect $query, $selector) {
		...
		if(!is_array($parent_ids)) $parent_ids = [$parent_ids]; 
	...
	protected function getQueryNumChildren(DatabaseQuerySelect $query, $selector) {
		...
		if(	(in_array($selector->operator, ['<', '<=', '!=']) && $value) || 
			(in_array($selector->operator, ['>', '>=', '!=']) && $value < 0) || 
	...
	protected function isModifierField($name) {
		...
		$alternates = ['checkAccess' => 'check_access', 'getTotal' => 'get_total', 'hasParent' => 'has_parent'];
		
		$modifiers = ['include', '_custom', 'limit', 'start', 'check_access', 'get_total', 'count', 'has_parent'];
	...
	protected function ___getQueryUnknownField($fieldName, array $data) { 
		...
		$_data = ['subfield ' => 'data', 'subfields' => 'data', 'fields' => [], 'query' => null, 'selector' => null, 'selectors' => null];
	...
	protected function getQueryOwnerField($fieldName, array $data): bool {
		...
		$ownerTypes = ['FieldtypeRepeater', 'FieldtypePageTable', 'FieldtypePage'];
