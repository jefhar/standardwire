...
class WireMarkupRegions extends Wire {
	 ...
	 * 
	 */
	protected $selfClosingTags = ['link', 'area', 'base', 'br', 'col', 'command', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'meta', 'param', 'source', 'track', 'wbr'];
	 ...
	 * 
	 */
	protected $actions = ['prepend', 'append', 'before', 'after', 'replace', 'remove'];
	 ...
	 * 
	 */
	public function find($selector, $markup, array $options = []) {
	
		if(strpos($selector, ',')) return $this->findMulti($selector, $markup, $options);
		
		$defaults = ['single' => false, 'verbose' => false, 'wrap' => null, 'max' => 500, 'exact' => false, 'leftover' => false];
	...
	protected function hasClass($class, array $classes) {
		...
		if(strpos($class, '*')) {
			...
			foreach($classes as $c) {
				if(str_starts_with((string) $c, $class)) {
					$has = $c;
					break;
				}
			}
	...
	protected function parseFindSelector($find) {
		...
		if($c === '#') {
			...
			$find = trim($find, '# ');
			foreach(['pw-id', 'data-pw-id', 'id'] as $attr) {
				$finds[] = " $attr=\"$find\"";
				$finds[] = " $attr='$find'";
				$finds[] = " $attr=$find ";
				$finds[] = " $attr=$find>";
		...
		*/
		...
		return ['tests' => $finds, 'selector' => $find, 'findTag' => $findTag, 'hasClass' => $hasClass];
	}
	...
	protected function getTagRegion($region, array $tagInfo, array $options) {
      ...
      'html' => '',
		...
		$closeQty = $selfClose ? 1 : substr_count($region, (string) $tagInfo['close']);
		...
		if(!$closeQty) {
			...
			$openTags = ["$openTag>", "$openTag ", "$openTag\n", "$openTag\r", "$openTag\t"]; 
			$fail = false;
			...
			do {
				...
				$closeCnt = substr_count($test, (string) $tagInfo['close']);
				if($openCnt == $closeCnt) {
					...
					$testStart = $testPos + strlen((string) $tagInfo['close']);
				}
	...
	public function getTagInfo($tag): array {
		...
		$info = ['id' => $id, 'pwid' => $pwid ?: $id, 'name' => $tagName, 'classes' => $classes, 'attrs' => $attrs, 'attrStr' => $attrStr, 'src' => $originalTag, 'tag' => "<$tag>", 'close' => $selfClosing ? "" : "</$tagName>", 'action' => $action, 'actionTarget' => $actionTarget, 'actionType' => $actionType];
	...
	public function stripRegions($tag, $markup, $getRegions = false) {
		...
		do {
			$pos = stripos((string) $markup, $open, $startPos);
			if($pos === false) break;	
			$endPos = stripos((string) $markup, $close, $pos); 		
			if($endPos === false) {
				$endPos = strlen((string) $markup);
			} else {
				$endPos += strlen($close);
			}
			$regions[] = substr((string) $markup, $pos, $endPos - $pos);
			$startPos = $endPos; 
		} while(1);
	...
	public function stripOptional($markup, $debug = false) {
		...
		foreach($attrs as $attrName) {
			...
			foreach($regions as $region) {
				...
				if($pos === false) continue;
				$content = trim((string) $region['region']);
				...
				if(strlen($content)) {
					...
					$markup = substr($markup, 0, $pos) . $open . substr($markup, $pos + strlen((string) $region['open']));
				} else {
					// empty optional region, can be removed
					$markup = substr($markup, 0, $pos) . substr($markup, $pos + strlen((string) $region['html']));
					if($debug) {
			...
			if(isset($attrs[$name])) {
				...
				if($name === 'class') {
					...
					foreach($classes as $key => $class) {
						if(!str_starts_with((string) $class, '-')) continue;
						$removeClass = ltrim((string) $class, '-');
						unset($classes[$key]);
	...
	public function renderAttributes(array $attrs, $encode = true, $quote = '"') {
		...
		foreach($attrs as $name => $value) {
			...
			if($value === true) {
				...
				$str .= "$name ";
				continue;
			} else if($name == 'class' && !strlen((string) $value)) {
				continue;
			}
				
			$q = $quote;
			if(!$q && !ctype_alnum((string) $value)) $q = '"';
	 ...
	 * 
	 */
	public function update($selector, $content, $markup, array $options = []) {
		
		$defaults = ['action' => 'auto', 'mergeAttr' => []];
		
		$options = array_merge($defaults, $options);
		
		$findOptions = ['verbose' => true, 'exact' => true];
	 ...
	 *
	 */
	public function replace($selector, $replace, $markup, array $options = []) {
	 ...
	 *
	 */
	public function append($selector, $content, $markup, array $options = []) {
	 ...
	 *
	 */
	public function prepend($selector, $content, $markup, array $options = []) {
	 ...
	 *
	 */
	public function before($selector, $content, $markup, array $options = []) {
	 ...
	 *
	 */
	public function after($selector, $content, $markup, array $options = []) {
	 ...
	 *
	 */
	public function remove($selector, $markup, array $options = []) {
	 ...
	 *
	 */
	public function populate(&$htmlDocument, $htmlRegions, array $options = []) {
		...
		$defaults = ['useClassActions' => false];
		...
		if(is_array($htmlRegions)) {
			...
			$regions = $this->find($selector, $htmlRegions, ['verbose' => true, 'leftover' => true, 'debugNote' => (isset($options['debugNote']) ? "$options[debugNote] => " : "") . "populate($callQty)"]);
			$leftoverMarkup = trim((string) $regions["leftover"]);
		...
		if(!count($regions)) {
			...
			if(strpos((string) $htmlDocument, 'pw-optional')) $htmlDocument = $this->stripOptional($htmlDocument, $debug);
			if($debug) {
		...
		$xregions = []; // regions that weren't populated
		$populatedNotes = [];
		$rejectedNotes = [];
		$updates = [];
		$numUpdates = 0;
		...
		foreach($regions as /* $regionKey => */ $region) {
			...
			if($debug) {
				...
				$open = $region['open'];
				$openLen = strlen((string) $open);
				if($openLen > 50) $open = substr((string) $open, 0, 30) . '[sm]... +' . ($openLen - 30) . ' bytes[/sm]>';
				$debugRegionStart = "[sm]" . trim(substr((string) $region['region'], 0, 80));  
				$pos = strrpos($debugRegionStart, '>');
				if($pos) $debugRegionStart = substr($debugRegionStart, 0, $pos+1);
				$debugRegionStart .= " … [b]" . strlen((string) $region['region']) . " bytes[/b][/sm]";
				//$debugRegionEnd = substr($region['region'], -30); 
				//$pos = strpos($debugRegionEnd, '</'); 
				//if($pos !== false) $debugRegionEnd = substr($debugRegionEnd, $pos);
				$region['note'] = strtoupper((string) $debugAction) . " [b]#{$pwid}[/b] " .
			...
			if(!$documentHasTarget) {
				...
				// update the markup
				$updates[] = ['actionTarget' => "#$actionTarget", 'regionHTML' => $regionHTML, 'action' => $action, 'mergeAttr' => $mergeAttr];
		...
		foreach($updates as $u) {
			$htmlDocument = $this->update($u['actionTarget'], $u['regionHTML'], $htmlDocument, ['action' => $u['action'], 'mergeAttr' => $u['mergeAttr']]);
		}

		$htmlRegions = trim((string) $htmlRegions);
		...
		if($recursionLevel === 1 && strlen((string) $htmlRegions) && $this->hasRegions($htmlRegions)) {
		...
		if(!$recursionLevel) {
			...
			if(strpos((string) $htmlDocument, 'pw-optional')) {
	...
	public function removeRegionTags(&$html) {
		...
		if(stripos((string) $html, ' data-pw-id=') || stripos((string) $html, ' pw-id=')) {
	...
	protected function renderDebugNotes(array $debugNotes, $debugTimer = null) {
		...
		$out = str_replace(['[sm]', '[/sm]'], ['<small style="opacity:0.7">', '</small>'], $out);
		$out = str_replace(['[b]', '[/b]'], ['<strong>', '</strong>'], $out);
