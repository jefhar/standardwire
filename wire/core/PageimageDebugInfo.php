...
class PageimageDebugInfo extends WireData {
 ...
 public function get($key) {
	...
	public function getBasicDebugInfo() {
		...
		if($depth < 2) {
			$info['variations'] = [];
			$variations = $this->pageimage->getVariations(['info' => true, 'verbose' => false]);
	 ...
	 *
	 */
	public function getVerboseDebugInfo($options = [], $returnType = 'string') {
		static $depth = 0;
		$depth++;
		...
		$thumb = ['thumb' => "<img src='$this->url' style='$thumbStyle' alt='' />"];
		
		if($this->original) {
			$original = ['original' => $this->original->basename, 'basename' => $this->basename]; 
		} else {
			$original = ['original' => '{SELF}', 'basename' => $this->basename];
		}
		...
		$info = array_merge($thumb, $parent,
			...
			['imageinfo' => ['imageType'   => $osInfo['info']['imageType'], 'mime'        => $osInfo['info']['mime'], 'width'       => $this->width, 'height'      => $this->height, 'focus'       => $this->hasFocus ? $this->focusStr : NULL, 'description' => $parent['files']['description'], 'tags'        => $parent['files']['tags']]]
		);
		...
		$webpInfo = ['webp_copy' => ['hasWebp'          => $webpSize ? true : false, 'webpUrl'          => (!$webpSize ? NULL : $webp->url()), 'webpQuality'      => (!isset($finalOptions['webpQuality']) ? NULL : $finalOptions['webpQuality']), 'filesize'         => $webpSize, 'savings'          => (!$webpSize ? 0 : intval($info['files']['filesize'] - $webpSize)), 'savings_percent'  => (!$webpSize ? 0 : 100 - intval($webpSize / ($info['files']['filesize'] / 100)))]];

		// ENGINES
		$a = [];
		$modules = $this->wire('modules');
		$engines = array_merge($oSizer->getEngines(), ['ImageSizerEngineGD']);
		...
		asort($a, SORT_STRING);
		$enginesArray = ['neededEngineSupport' => strtoupper($oSizer->getImageInfo()), 'installedEngines' => $a, 'selectedEngine' => $oSizer->getEngine()->className, 'engineWebpSupport' => $oSizer->getEngine()->supported('webp')];
		...
		$info = array_merge($info, $webpInfo,
			...
			// OPTIONS
			['options_hierarchy' => ['imageSizerOptions' => $this->wire('config')->imageSizerOptions, 'individualOptions' => $options, 'finalOptions' => $finalOptions]]
		);
